...
type SelectItem = { value: unknown; label: React.ReactNode }
const SelectItemsContext = React.createContext<Array<SelectItem> | undefined>(undefined)

function Select<T>({
  items,
  ...props
}: SelectPrimitive.Root.Props<T> & { items?: Array<SelectItem> }) {
  return (
  ...
  return (
    ...
    >
      ...
      <SelectPrimitive.Icon
        render={
          <Icon icon="lucide:chevron-down" className="text-muted-foreground size-4 pointer-events-none" />
        }
      />
    </SelectPrimitive.Trigger>
  )
